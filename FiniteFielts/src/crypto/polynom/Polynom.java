...
public class Polynom {
    ...
    /**
     * @param p
     *            has to be a Prim number, is represending the modulo Value for the Polynoms
     * @param n
     *            has to be a number, is represending the degree of the Polynoms
     * @return a List out of all possible Polynoms to generate a modulo Field sized p^n
     */
    public static List<Polynom> createGeneratingPolynomes(int p, int n) {
        Preconditions.checkArgument(n > 1, "n must be greater than 1");
        Preconditions.checkArgument(Polynoms.isPrime(p), "p musst be prim");
     ...
     *            has to be a number, is represending the degree of the Polynoms
     * @param x
     *            represents the max number of created polynoms it always returns only as much as existing
     * @return a List out of X or all possible Polynoms to generate a modulo Field sized p^n
     */
    public static List<Polynom> createXGeneratingPolynome(int p, int n, int x) {
        Preconditions.checkArgument(n > 1, "n must be greater than 1");
        Preconditions.checkArgument(Polynoms.isPrime(p), "p musst be prim");
        Preconditions.checkArgument(x > 0, "max count must be greater than 0");
        List<Polynom> allGeneratingPolynomes = new ArrayList<Polynom>();
    ...
    public static Polynom createPolynomFromVectorString(String polynom, int p) {
        Preconditions.checkArgument(Polynoms.isValidPolyVektorString(polynom),
                "polynom has to be a Vektor, something like (1,2,2)");
        Preconditions.checkArgument(Polynoms.isPrime(p), "p musst be prim");

        polynom = polynom.substring(1, polynom.length() - 1);
        String[] polynomStringNumbers = polynom.split(",");
        ...
        return createPolyFromArray(polynomNumbers, p);
    }
    ...
    /**
     * returns a new Polynom that is calculated be (Polynom1 + Polynom2) modulo p. The modulo of both Polynoms has to be
     * equal.
     */
